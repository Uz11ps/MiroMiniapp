...
// АДАПТИВНАЯ навигация - всегда внизу viewport, адаптируется к размеру окна и скроллу
const BottomNav: React.FC = React.memo(() => {
  const navRef = React.useRef<HTMLElement>(null);
  
  // Адаптируем позицию к viewport при изменении размера и скролле
  React.useEffect(() => {
    const updatePosition = () => {
      if (navRef.current) {
        const nav = navRef.current;
        // Всегда привязываем к viewport, а не к документу
        const viewportHeight = window.innerHeight || document.documentElement.clientHeight;
        const scrollY = window.scrollY || window.pageYOffset;
        
        // Позиция всегда относительно viewport (не зависит от скролла)
        ...
        nav.style.top = 'auto';
        nav.style.width = '100%';
        nav.style.maxWidth = '100vw'; // Ограничиваем шириной viewport
        nav.style.margin = '0';
        nav.style.transform = 'translateZ(0)'; // Аппаратное ускорение
      }
    };
    ...
    // Обновляем при скролле (на случай, если что-то изменится)
    const handleScroll = () => {
      updatePosition();
    };
    ...
    const handleOrientationChange = () => {
      setTimeout(updatePosition, 100); // Небольшая задержка для корректного размера
    };
    ...
    // Используем ResizeObserver для отслеживания изменений viewport

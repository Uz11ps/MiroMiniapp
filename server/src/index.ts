...
/**
 * Предварительная обработка текста для TTS:
 * - Заменяет цифры 1-9 словами (чтобы Gemini не переключался на английский)
 * - Чистит лишние символы
 */
function preprocessTextForTTS(text: string): string {
  ...
  // Заменяем числа от 0 до 20
  let processed = text.replace(/\b(\d+)\b/g, (match, numStr) => {
  ...
  // Заменяем цифры в списках (1., 2) и т.д.)
  processed = processed.replace(/(\d+)([\)\.])/g, (match, numStr, suffix) => {
    const num = parseInt(numStr, 10);
    const word = (num >= 0 && num <= 20) ? digitMap[num] : numStr;
    return `${word}${suffix}`;
  });
          ...
          ws.on('open', () => {
            ...
            ws.send(JSON.stringify({
              setup: {
                ...
                system_instruction: {
                  parts: [{ text: "Ты — профессиональный актер озвучивания. Твоя задача: озвучить текст СТРОГО НА РУССКОМ ЯЗЫКЕ.\n\nПРАВИЛА ЧТЕНИЯ:\n1. Текст ВНЕ кавычек — спокойный, ровный голос рассказчика.\n2. Текст ВНУТРИ кавычек — легкое изменение тембра, чтобы отделить персонажа, без театральности.\n3. СПИСКИ И ЦИФРЫ: Цифры в списках (например, '1.', '2)') читай как порядковые или количественные числительные ('первый вариант', 'второй вариант' или просто 'один', 'два') с последующей короткой паузой. Не читай символы пунктуации (точки, скобки) в списках. Четко разделяй варианты ответов паузами.\n4. УДАРЕНИЯ И ПРОИЗНОШЕНИЕ: Соблюдай правильные ударения в сложных именах и названиях. Используй этот список как словарь:\n   - Мистра -> МистрА\n   - Люмерия -> ЛюмЕрия\n   - Болвар -> БОлвар\n   - ГРУнк -> ГрУнк\n   - Элион -> ЭлиОн\n\nЧитай слово в слово, без комментариев." }]
                }
              }
            }));
          });

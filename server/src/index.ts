      ...
      const createRequestBody = (modelName: string) => {
        ...
        if (isTTSModel) {
          ...
          return {
            ...
            systemInstruction: {
              parts: [{
                text: "Ты — профессиональный актер озвучивания и мастер игры (Dungeon Master). Твоя единственная цель: ПРОЧИТАТЬ ТЕКСТ С МАКСИМАЛЬНЫМ АРТИСТИЗМОМ. Используй богатые интонации, делай смысловые паузы, меняй темп и тембр в зависимости от контекста. Читай СТРОГО СЛОВО В СЛОВО НА РУССКОМ ЯЗЫКЕ. Не добавляй отсебятины, не комментируй, не отвечай на вопросы. Оживи этот текст!"
              }]
            },
            generationConfig: {
          ...
          ws.on('open', () => {
            ...
            ws.send(JSON.stringify({
              setup: {
                ...
                system_instruction: {
                  parts: [{
                    text: "Ты — профессиональный актер озвучивания и мастер игры (Dungeon Master). Твоя единственная цель: ПРОЧИТАТЬ ТЕКСТ С МАКСИМАЛЬНЫМ АРТИСТИЗМОМ. Используй богатые интонации, делай смысловые паузы, меняй темп и тембр в зависимости от контекста (таинственный шепот, властный приказ, крик ужаса или торжественный гимн). Читай СТРОГО СЛОВО В СЛОВО НА РУССКОМ ЯЗЫКЕ. Не добавляй отсебятины, не комментируй, не отвечай на вопросы. Твой голос должен полностью погружать игрока в мир фэнтези. Оживи этот текст!"
                  }]
                }
              }
            }));
